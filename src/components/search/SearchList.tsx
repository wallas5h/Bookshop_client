...
export const SearchList = () => {
  ...
  const { phrase, currentPage } = useSelector((store: RootState) => store.search);
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch(`${apiUrl}/book/search/?${generateQueryString({ phrase, currentPage, perPage })}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  return (
    <>
      <section>
        <p className="search-result">Results for:<span>"{phrase}" </span></p>
        <div className="search-result--count">
          <p><strong>{resultsCount}</strong> results</p>
        </div>
        ...
        <div className="search-result-view-per-page">
          <label>
            ...
            <select value={perPage} onChange={(e) => setPerPage(Number(e.target.value))}>
              <option value="5" defaultValue={10} >5</option>
